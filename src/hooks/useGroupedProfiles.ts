...
export const useGroupedProfiles = (
  ...
  useMemo(() => groupProfiles(structure, profiles), [structure, profiles]);
      ...
      departments: divDepts
        .map((dept) => {
          const head =
            ...
            (profiles ?? []).find((p) =>
              (p.assignments ?? []).some(
                (a) => a.role === HEAD_OF_DEPARTMENT && a.department === dept.name
              )
            ) ??
            null;
  ...
  const campusByUniversity = campusUniversities
    .map((uni) => ({
      ...
      profiles: campusProfiles
        .filter((p) => (p.assignments ?? []).some((a) => a.university === uni))
        .sort((a, b) => {
