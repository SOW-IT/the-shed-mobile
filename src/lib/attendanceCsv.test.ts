...
describe("buildAttendanceMatrixCsv", () => {
  ...
  test("an event with no attendance still gets a zero total and an empty column", () => {
    ...
    // Ada attended only Week 1 of 2 → 50%.
    expect(lines[2]).toBe("Ada Lovelace,ada@sow.org.au,50%,,Y");
  });
