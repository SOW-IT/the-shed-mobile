...
- Emits per year: divisions, departments (head resolved from uid to email),
  universities (the distinct values used that year), the per-year role catalog
  (the roles/<year> doc when present, else the distinct roles used that year),
  the Budget Manager's email, and one profile per person with
  roles/department/division/university. Years before divisions existed get a
...
// Per-year `roles` were backfilled from the Firestore roles/<year> collection
// (scripts/roles_by_year.json) since that dump isn't checked into the repo;
// years it doesn't cover use the distinct roles actually used by that year.
...
for y in payload_years:
    print(
        ...
        f"{len(y['universities'])} unis, {len(y['roles'])} roles, "
        f"BM={y.get('budgetManagerEmail')}"
    )
